...
NSString * Figueroa_Austin_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Figueroa_Austin_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_figueroa_austin_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Figueroa_Austin_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Figueroa_Austin_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_figueroa_austin_project3 = [[Figueroa_Austin_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_figueroa_austin_project3];
	
	NSString *_figueroa_austin_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _figueroa_austin_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_figueroa_austin_project3 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Figueroa_Austin_Project3$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.figueroa_austin_project3.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.figueroa_austin_project3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
